...
// 왜 product클래스도 import하면 에러가 뜨는 것일까?
...
void main() {
  ...
  while (isRunning) {
    ...
    print(
        '--------------------------------------------------------------------------------------------------------------------------------------------------------');
    print(
        '[1] 상품 목록 보기 / [2] 장바구니에 담기 / [3] 장바구니에 담긴 상품의 총 가격 보기 / [4] 프로그램 종료 확인 / [5] 프로그램 종료 / [6] 장바구니 초기화');
    print(
        '--------------------------------------------------------------------------------------------------------------------------------------------------------');
    // print("-" * 100); 와 같은 다양한 표현 방식 생각해보기!

    String? choice = stdin.readLineSync(); // 사용자 입력 (혼란의 영역)
    ...
    switch (choice) {
      ...
      case '4':
        // 종료 확인 메시지 출력
        print("정말 종료하시겠습니까? (5 입력 시 종료, 다른 값 입력 시 계속)");
      ...
      case '6':
        // 장바구니 초기화
        if (mall.isCartEmpty()) {
          print("이미 장바구니가 비어있습니다."); // 장바구니가 비어있을 경우 처리
        } else {
          mall.clearCart(); // 장바구니 초기화
          print("장바구니를 초기화합니다."); // 초기화 메시지 출력
        } // "장바구니를 초기화합니다."의 이중출력이 여기와의 충돌인지 공부 및 확인 필요!
        break;
